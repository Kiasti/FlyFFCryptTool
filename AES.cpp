...
bool CheckLargeTables(const bool create)
{
	...
	{
		unsigned char a1 = byte_sub[i];
		const unsigned char a2 = xmult(a1);
		const unsigned char a3 = a2 ^ a1;

		const unsigned char b5 = inv_byte_sub[i];
		const unsigned char b1 = GF2_8_mult(0x0E, b5);
		const unsigned char b2 = GF2_8_mult(0x09, b5);
		const unsigned char b3 = GF2_8_mult(0x0D, b5);
		const unsigned char b4 = GF2_8_mult(0x0B, b5);
...
unsigned long SubByte(unsigned long data)
	...
	unsigned result = byte_sub[data>>24];
	result <<= 8;
